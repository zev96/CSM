 ...
 * 都不再关闭弹窗 —— 这几个阶段都是任务态，让用户走显式按钮决策。Dialog
 * 的 ``closable`` prop 实现这个 gating。
 *
...
function onOpenChange(open: boolean) {
  // Dialog 只有在 closable=true 时才会 emit update:open=false（来自
  // backdrop click 或 esc）。下面 closable 表达式保证只有 prompt 阶段
  // 会走到这里 —— 等价于"点遮罩 = prompt cancel"，其它任务态不会触发。
  if (!open) onPromptCancel();
}

// 只有 prompt 阶段能通过 backdrop / esc 关闭。
const closable = computed(() => updateAlertState.phase === "prompt");
...
<template>
  <Dialog
    v-if="updateAlertState.info"
    :open="updateAlertState.open"
    size="lg"
    :closable="closable"
