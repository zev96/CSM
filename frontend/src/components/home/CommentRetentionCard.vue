...
// 主选中行：取留存率最低的（出问题的最值得关注）。没有 row 时 null。
const topRow = computed<Row | null>(() => {
  if (rows.value.length === 0) return null;
  return [...rows.value].sort((a, b) => ratio(a) - ratio(b))[0];
});
const restRows = computed(() =>
  rows.value.filter((r) => r.key !== topRow.value?.key),
);
  ...
  >
    ...
    <div v-else class="flex min-h-0 flex-1 flex-col gap-1 overflow-y-auto">
      ...
      >
        ...
        <span
          class="inline-flex h-5 flex-shrink-0 items-center gap-0.5 rounded-full px-2 text-[10.5px] font-medium"
          :style="rowChipStyle(r)"
        >
          <Icon
            :name="rowChipKind(r) === 'down' ? 'arrowDown' : 'x'"
            :size="9"
          />
          {{ rowChipText(r) }}
        </span>
