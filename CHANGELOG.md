...
### 移除
- **数据中心页面下线**（左侧导航「数据中心」及其 5 个分析子页：平台评论留存 / 知乎问题 / 知乎搜索 / 百度排名 / GEO 覆盖榜）。监测中心任务详情内的分析与首页统计卡不受影响，`/api/monitor/history/*` 接口保留。
- **创作区「组装」预览 tab 下线**：起飞后直接落到「初稿」，segmented 切换只剩「初稿 / 成稿」；单段重随 / 全部重采 / 锁结构入口随之移除（右侧面板的「重新随机」仍可整篇重抽）。采样告警（竞品缺料 / 素材不足）改在初稿头部显示。
...
- **sidecar 体积**：PyInstaller 排除 scipy（只被 datasketch 的包初始化链带进来，运行期唯一用到的 `integrate.quad` 用一个等价的复合 Simpson 替身提供，单测逐项核对 LSH 参数一致）与 pygments，单文件 sidecar 缩小约 16MB。
