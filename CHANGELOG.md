...
- **竞品池卡片模式「从目录识别小节」**：配小节时不用再去 Obsidian 里逐个抄卡片的 `##` 标题——竞品池面板新增「从目录识别」按钮，扫一遍该目录里的竞品卡，列出它们实际写了哪些小节、每个小节有几篇写了正文（只有标题没正文的卡进不了名册，这一栏一眼看出还差多少篇），可勾选后「按目录替换」或「只补未配置的」，并直接给出「按当前勾选最多几张卡能上榜」。识别只读不落库，先看再导，不会静默覆盖你调过的必需/候选数。（顺带隐藏了卡片模式下不生效的「子素材随机数量 / 不重复素材」两个开关。）

### Changed

- ⚠️ **口径变更 · 「重新随机」现在会换结构版本**：模板配了多套结构版本时，此前点「全部重采 / 重新随机」会**锁住当前版本、只换文字素材**，于是第一次抽中哪套结构就一直停在那套（还叠加了一个种子相关性 bug，实测一直卡在版本2 换不出来）。现在「重新随机」会在各版本之间**自由重抽**；如果你想保留当前结构、只换个说法，用新增的「只换文字·锁结构」入口。
- ⚠️ **口径变更 · 筛选条件的匹配放宽**：模板筛选此前是严格相等，于是两类资料库里很常见的写法**永远筛不出东西**——标签列表（`核心关键词: [模板二, 主推位, 品牌实力]`，选「品牌实力」匹配不上整个列表）和数字/日期（`模板序号: 2` 是数字、`日期: 2026-06-26` 是日期，填的文本严格比不等）。现在改为**按每篇笔记自己的类型判定**：列表看「是否含这个标签」，其余按文本归一后比对。匹配只会变多不会变少（第一步仍是严格相等，旧代码筛到的一篇都不会丢），因此**用到列表/数字/日期字段的筛选，其命中结果可能变化**——多数是「本来一篇都筛不出→现在筛得出」，但如果某个字段在同一目录里混着标量和列表两种写法，也可能「本来筛出 N 篇→现在更多」。经核对，当前所有模板对你的资料库都只筛标量字符串字段（素材类型/模块/核心关键词在其所在目录都是标量），放宽对它们逐字节等价、产出不变；上面的变化只影响将来用到列表/数字/日期字段的筛选。

### Fixed

- **明明有素材却报「没有符合条件的素材」时，现在会说清差在哪**：报错会补一句归因——该目录下有几篇素材、你填的那个筛选字段实际取值是什么、以及**你要的那个值其实在哪个字段里**（最常见的就是字段名填错：值写「品牌实力」，字段该填「模块」却填成了「素材类型」）。字段整个不存在时会列出该目录真实用到的字段；目录本身是空的、或多条筛选各自能命中但没有素材同时满足，也分别说明。主推卡小节与竞品卡池两条路径都生效。
- **主推卡小节的筛选不用再手敲**：「筛选字段 / 筛选值」两格改成从资料库聚合的下拉（与段落、对比池的筛选一致），列出该目录笔记里真实存在的字段与取值，素材还没写时可以选「＋ 自定义属性…」手填。同时去掉了字段那格的默认值「素材类型」——它是个幽灵默认：用户从没选过，填上值却会一起落库，配出一个恒空的筛选。空的现在显示「不筛选」。
- **竞品池开了卡片模式后可能存不下去**：卡片模式要求填「筛选条件」（否则同目录下的旧格式竞品笔记会混进名册），但筛选属性只能从下拉里选，而下拉是按当前目录已有素材聚合的——竞品卡还没写时下拉是空的，等于无论如何都存不了。现在开启卡片模式会自动填入约定值（素材类型 = 竞品卡）；已经卡住的块会就地提示并给一键补填；属性下拉也新增「＋ 自定义属性…」可以手填，不再依赖素材先存在。
- **主推卡文末的「取材 / 红线」说明不再渗进正文**：主推位素材笔记把「取材 / 红线」这类给作者看的说明块排在「返回上层」导航之上，此前只从「返回」那一行往下截断，导致排在它上面的说明块被当成最后一段正文录进成稿（文章里会冒出「取材:[[..]] 红线:」整块）。现在从最先出现的任意说明/导航标记处截断，与说明块和导航的先后顺序无关。
