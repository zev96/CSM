...
### 新增
- **引流抓取：自动跳过「评论精选后才可见」的视频（B站）**：UP 主开启评论精选后（评论框提示「评论被up主精选后，对所有人可见」），访客发的评论要被 UP 主手动精选才公开——引流评论发了也没人看得见。现在抓取完成后会逐条识别这类视频并自动排除，不再进候选列表；被排除的视频留在库里，后续任务靠全局去重直接跳过，不会反复抓到。填了品牌词的任务搭品牌预筛抓评论的便车识别（零额外请求；那次抓取偶发失败时回落轻量探测补判）；没填品牌词的任务走轻量探测——每条视频 1 个匿名小请求、间隔 1–2.5 秒，50 条约多 1.5 分钟，不带登录 cookie、不消耗账号风控额度；B站对匿名请求偶发的 `-352` 风控抽检会自动重试一次，连续失败 3 条则熔断、其余视频照常保留（不误删）。跳过了几条会写进任务状态提示（悬停状态标签可见，如「3 条开启了评论精选（评论需博主精选后才可见），已自动跳过」），任务完成通知里也会报数。不想要这个行为可在 `settings.json` 里把 `mining_skip_featured_comments` 设为 `false`。

### 说明
- 抖音 / 快手 / 小红书暂未支持：手头的真实接口响应都是普通视频，找不到可验证的「精选」字段，没有开启精选的样本可对照，不做猜测式判断（误判会白丢候选视频）。遇到这类视频请把链接留下来，拿到样本确认字段后即可补上。
...
- **引流抓取：设了 10 条为什么列表里只有 7 条**：被全局去重跳过（已在评论监控任务里 / 之前任务已采集过）的条数会写进任务状态提示（悬停状态标签可见，如「3 条已在监控/已采集过，未重复入库」）；进度仍按适配器交付数计到 10/10，不会因去重卡住。取消后被跳过的平台也会推「已取消」事件，托盘不再显示 0/N。
...
- **引流抓取新增小红书平台**：新建抓取任务的平台卡多了「小红书」（仅 TikHub 采集模式可选；浏览器模式显示「仅支持 TikHub 采集」），走 TikHub `app_v2/search_notes`，筛选项：发布时间档位（不限 / 一天内 / 一周内 / 半年内）、笔记类型（不限 / 视频 / 图文）、排序（综合 / 最多点赞 / 最多评论 / 最多收藏 / 最新），全部服务端下推。搜到的笔记规范链接为 `xiaohongshu.com/explore/{note_id}`，结果里带 `xsec_token` 时一并拼进链接（网页端没有它打不开笔记）。品牌评论预筛对小红书同样走 TikHub 评论接口（缺 Key 时跳过不排除）；候选池平台筛选、任务卡、CSV 导出平台列、评论模板库来源平台、同步监测都已认识小红书；腾讯文档同步默认把小红书路由到名为「小红书」的子表。
- **腾讯文档表头识别 + 逐列确认**：设置 → 腾讯文档同步里粘贴表格链接并保存后，自动读取表格里**每一张子表**的表头，按列名识别「链接 / 序号 / 日期 / 第 N 层评论 / 第 N 层图片」并标出各平台会写进哪张子表——只认列名不认位置，评论列和图片列不相邻、顺序打乱、只有 3 层都能正确对上。识别惯例放宽到 `评论A / 评论1 / 评论一 / 一楼 / 第一层评论 / 楼层1 / 内容一 / 盖楼内容二` 与 `评论A的图片 / 评论1图片 / 一楼图片 / 贴图一 / 图片1 / 第1层图片 / 配图2`，`截图 / 返图 / 执行状态` 等兼职回填列明确排除。识别不准的列可在面板里逐列改成正确角色并「保存映射」，之后同步严格按这份映射写（同一角色指到两列会拦下）；「恢复自动识别」可随时撤销。表头后来改动过（列挪位 / 改名）时，旧映射自动作废、退回自动识别，并在同步结果里提醒重新确认。面板还会提示该表只有几层评论列、哪一层没有图片列。
- **腾讯文档同步：评论楼层的图片直接插进表格**：审核通过的评论如果在 app 里挂了图，同步时不再只写「有图，另发」，而是把图片本体（base64）插进对应楼层的「评论X的图片」/「贴图N」列（服务端 sheet-mcp `insert_image`；一格多图会叠放在同一格，可拖开）。服务端没有该工具、或某张插入失败（文件缺失 / 超 4MB / 服务端拒绝）时，该格回落写「有图，另发」标记，绝不让一条有图评论在表格里"看起来没图"。设置 → 腾讯文档同步新增「评论图片直接插进表格」开关（默认开），「测试连接」会显示服务端是否支持插图；同步结果提示新增已插入 / 失败 / 未直传的图片张数。

### 说明
- 小红书两个 TikHub 端点（评论 / 搜索）的响应字段路径按小红书 App 接口的公开形态实现并逐层兜底，尚未像抖音 / B站 / 快手那样用真实 token 落 fixture 实测；首次使用建议先跑 `sidecar/scripts/tikhub_probe.py --xhs <笔记链接> --xhs-search <关键词>` 各抓一份响应，对照归一化函数校正（对应测试文件末尾的 real-fixture 用例会自动生效）。

## [0.8.2] - 2026-09-04

### 新增
- **采集改走 TikHub 付费搜索**：关键词找视频（抖音 / B站 / 快手）默认走 TikHub 付费 API —— 免登录、免并发风控。每页 $0.01、约 6–14 条，单平台上限 80 条/次、翻页硬闸 12 页；页级失败重试（余额不足 / 鉴权失败 / 服务端已出货的业务错误不重试），首页失败记失败、后续页失败记完成并附 note。设置页新增「采集走 TikHub 付费搜索」开关（默认开），浏览器采集保留为手动兜底；任务弹窗平台卡在 TikHub 模式显示「TikHub 就绪」并默认全选，未配置 Key 时显示「未配置 TikHub Key」且不可选。⚠️ **升级即默认走付费 API**：请确认已在「设置 › 监测 › 抓取数据源」粘贴 TikHub API Key，或关闭该开关改回浏览器采集。
- **腾讯文档同步按表头惯例自动对列**：`评论A / 评论A的图片 / 评论B / 评论C …` 自动映射为各层评论与配图，写入层数由表头列数决定（按列存在性逐层判定，表头断层不会假标已同步）；链接列兼容 `链接 / 视频链接 / 文章链接`；「测试连接」显示识别层数、评论列断层与可选列告警。评论生成层数上限 3 → 5。
- 任务卡片：平台采集结束时的说明（如「第 N 页失败已停止」「已翻 N 页 0 条命中」）现在会显示在状态提示里。

### 修复
- **腾讯文档「测试连接」报 `tool not found: sheet.get_sheet_info`**：工具名误加 `sheet.` 前缀（照抄了 mcporter 的「服务名.工具名」CLI 写法），真服务工具名不带前缀；已改为 `get_sheet_info / get_cell_data / set_range_value_by_csv / set_cell_style`。「测试连接」新增 `tools/list` 诊断，失败时列出服务端真实工具名。
- 采集：第二、三个平台运行中没有实时进度事件（进度节流状态未按平台重置）；平台异常退出时进度写成 0 条。
- TikHub 客户端：GET 日志不再记录参数值（关键词）；HTTP 200 但响应非法 JSON 不再重试计费。

## [0.8.1] - 2026-09-01

### Fixed

- **更新检查全员失败（auth failed: GitHub returned HTTP 401）**：v0.8.0 及更早的安装包里烙着一个发布流程注入的 GitHub 访问令牌（仓库还是私有时的遗留设计），令牌过期后 GitHub 对带无效凭证的请求一律回 401——所有装机同时失去「检查更新」能力，与仓库本身无关。两层修复：① 发布流程不再注入令牌（仓库已公开，匿名即可读版本信息、下载安装包）；② 客户端带令牌收到 401/403 时自动摘掉凭证匿名重试一次，更新检查 / 清单获取 / 安装包下载三条路径同策略，凭证过期从此不再打断更新链路。**已装 v0.8.0 及更早版本的用户需手动下载本版安装包覆盖安装一次**（旧版的应用内更新检查已无法工作），之后恢复正常应用内热更新。

## [0.8.0] - 2026-09-01

### Added

- **引流抓取支持按平台筛选搜索条件**：新建任务弹窗按勾选的平台分别展开各自真实支持的筛选项——B站可选排序（综合 / 播放 / 最新 / 收藏）和任意发布日期区间；抖音可选发布时间档位（不限 / 一天内 / 一周内 / 半年内）、排序（综合 / 最多点赞 / 最新）和内容类型（视频 / **图文**，图文走综合搜索自动识别）；快手平台没有官方时间筛选，给日期区间后抓取时按发布时间本地过滤（被滤掉的不占数量，自动多翻页补齐，产出会略慢）。每个任务用了什么筛选条件会存档，可追溯。
- **品牌评论预筛口径升级 + 热评留档**：预筛从「前 30 条命中 3 条才排除」收紧为「**评论区前 20 条命中品牌词 1 条即排除**」（两个参数都可在 settings.json 的 `mining_prefilter_*` 里调）；抖音的评论抓取自动改走 TikHub API（本地路径签名不可用；配置了 TikHub key 即自动启用，B站 / 快手继续走免费本地路径省额度）。预筛抓到的前 20 条热评会随视频存档——既是排除证据（能看到命中了哪条），也是后续 AI 生成评论的评论区语料，不用二次抓取。
- **AI 批量生成评论 + 人工审核队列**：勾选任意多条视频，点浮动工具条上的「AI 生成」即可批量产出评论草稿——评论模板库自动轮换（星标 / 常用优先），改写时结合视频标题、播放数据、评论区热评样本和 AI 速览，要求贴视频具体元素、模仿评论区语言风格、禁用 AI 套话、不虚构可证伪细节；可选盖楼 1-3 层（第 2 层起与前层形成对话感），生成结果先过一遍确定性 AI 味检查，检出套话自动带着违规项重写一次，仍超标的在界面挂黄色「AI 味」警示。生成的草稿全部进入**待审核**队列（视频列表新增「待审核」筛选和「待审 N」徽标）：逐条「通过」、批量通过、或**直接改文案保存即视为通过**；不满意就再点一次 AI 生成覆盖重来（人工写的和已通过的楼层绝不会被覆盖）。改写提示词可在「设置 → Outreach AI 提示词」自定义。
- **审核通过的评论一键同步腾讯文档**：引流页右上角新增「同步腾讯文档」——把全部已通过的评论追加到与兼职共享的在线表格。表格按平台分子表时（抖音 / B站 / 快手 tab）自动把每个平台的视频写进对应子表；列位置按**表头列名**自动对齐（序号 / 链接 / 内容一 / 贴图一 / 盖楼内容二…），调整列顺序不影响；序号每批从 1 重排、批与批之间自动留一行橙色分隔行（沿用表内惯例，已有的尾部分隔行会复用不重复）；评论带图的行在贴图列标「有图，另发」（图片继续走手机直发兼职）。防重复写入有三道保险：本地状态门禁、写入前回读表格链接列去重、同步批次对账表。配置入口在「设置 → 腾讯文档同步」：粘贴表格链接和腾讯文档 Skill 的 Token（只存本机系统钥匙串），「测试连接」会逐平台显示路由到哪张子表、列映射是否齐全。未配置或同步失败时，原有的 CSV 导出兜底不变。

## [0.7.13] - 2026-08-31

### Added

- **评论监测批次可一键导出留存率报告**：抖音 / B站 / 快手的评论监测批次新增「导出留存率」入口（批次「⋯」菜单，以及点进批次后的头部下载按钮），弹原生「另存为」选位置，导出 Excel 双击直开的 CSV。文件头部是汇总统计——总计多少条、留存多少条、留存率百分比、各状态分布（在显 / 跌出理想 / 被删除 / 超出检索 / 未监测 / 监测失败）；下面每条链接一行：视频链接、我的评论、是否留存、当前排第几名（命中给纯数字方便在 Excel 里排序，超出检索深度显示「150+」）、检索深度、评论区总数、最近检查时间和一句人话说明（如「上次还在前 150 名，这次没找到——评论可能已被删除」）。口径与界面完全一致：行单位 = 链接+评论，留存 = 在显 + 跌出理想。

## [0.7.12] - 2026-07-30

### Added

- ⚠️ **关键词自然植入引言和结尾**：过去你输入的关键词只有标题受保护，正文里 AI 爱写不写——引言/结尾来自素材库，往往通篇不含这个搜索词。现在整篇润色（以及链上的每轮精修）都会要求 AI 检查引言和结尾：没出现关键词的各自然融入一次（贴合上下文、不生硬堆砌），已出现的保持原样不重复追加。**口径变更**：同样的操作，升级后的成稿引言/结尾会带上关键词；单篇整篇润色和批量生成都生效。配套地，关键词本身纳入了事实核对白名单（单篇与批量同口径）——像「2000元以内空气净化器推荐」这种带**数字+单位**的关键词织进正文后，不会被事实核对当成越界参数拦下或扣分（纯年份等裸数字本来就不在核对范围内）。
- **润色被保护性回退时不再静默**：润色链上有两道守卫——排版守卫（榜单卡片被润成流水文时整轮回退）和标题守卫（标题被改写/删除时纠正回来）。过去它们只写后端日志，用户拿回一篇和初稿几乎一样的「成稿」，只会以为润色没干活。现在哪一轮被回退/纠正，会在「润色过程」弹窗里逐轮标黄说明，润色完成时也会弹一条提示。

### Fixed

- **起飞后在右栏换的「AI 润色 Skill」不生效**：创作区右栏那个下拉改的只是界面显示，「整篇润色」实际用的一直是**起飞那一刻**的参数快照——而从首页起飞根本不带单选 skill，于是链等于没带任何人设在跑：system prompt 是空的，AI 只做删敏感词、调标点级别的最小修改，你设置的润色 skill 从头到尾没参与（顶栏那个「persona」小标签就是这么来的——链里没有 skill 名可显示，只好显示角色名兜底）。现在「整篇润色」以界面**当前所选**为准（skill 链非空时链优先，和起飞的规则一致），换完 skill 点润色立刻生效；不带参数直接从左侧导航进创作区时，右栏也会回填上次起飞的选择，不会把已选的 skill 静默降级成空。

## [0.7.11] - 2026-07-28

### Added

- **竞品小节「变体不足」提示**：竞品部分的随机有两层——抽哪几个竞品、每个竞品的每个小节抽哪条文案。第二层的上限完全由素材决定：某个小节如果全部竞品卡都只写了 1 条，那它在每篇文章里都是同一段话，而这既不报错也不算缺料，没人知道该去补哪一节。现在生成结果的告警区会点名这类小节（例：「小节『综合总分』的 56 张竞品卡都只写了 1 条变体，每次生成这一节都是同一段话」）。只在**全部**竞品卡都是单条时才提示，有任意一张写了两条就不打扰。

### Fixed

- **引言等素材把「关联素材: [[…]] | [[…]]」这类说明行当成正文写进文章**：笔记末尾那块给作者自己看的导航/引用（返回上层、取材、红线、关联素材…）本该整块丢弃，但识别方式是**写死的标签名单**，名单里没有的标签就漏网——「关联素材」正是漏的那个，于是切割点滑到下一行的「返回上层」，中间那行连同分隔线被当作最后一段正文录进成稿。现在改为按**结构**识别：一行的内容除了指向其他笔记的链接什么都没有，它就是引用/导航，不是正文，标签叫什么都拦得住；说明块内部的先后顺序也不再影响判定。经全库 764 篇逐篇核对，清掉了 **22 篇素材**的残留（空气净化器引言模块的人设/痛点/市场乱象/过渡连接共 19 篇、品牌背书 1 篇、整机推荐文案 2 篇），**素材条数、卡片小节数均无变化**，没有任何一行正文被误删。
  - 适用边界：本次治的是「整行只有链接」这一种说明块。写在正文**开头**的散文型批注（范文笔记的「⚠️ 取用前必读」「使用注意」整段）不在此列——按现有模板生成碰不到它们（实测 320 篇成稿零出现），但如果你新建一个直接取「整机推荐文案」目录的模板块，那段告示会被当正文写进文章。这 2 篇范文素材建议在资料库里把告示移出正文。
  - 另外：几篇「总索引」笔记文末的导航清单（如「挑选攻略 9 篇：[[…]]、[[…]]」）现在也一并归为说明块。它们只是目录里的跳转入口，不参与文章生成，故对成稿无影响。
- **素材随机组合失灵——不同关键词的文章都是同一套素材**：素材抽取本身是按随机种子组合的，但界面把种子写死了：单篇起飞永远发同一个值，批量则整批所有关键词共用一个（老版桌面端的「每次随机滚种子」在换新界面时丢了）。结果同一模板下不管换什么关键词、单篇还是批量，抽中的素材和各小节内容都逐字节一样，看起来就是「素材固定、按顺序往下铺」。现在：① 单篇每次起飞都随机滚新种子，同一个关键词反复生成，素材组合也各不相同；② 「全部重采 / 只换文字·锁结构」同样滚新种子，不再是固定的轮换序列；③ 批量里每个关键词各自随机抽素材和结构版本（同一个词的多候选仍落在同一结构版本内比稿，评分口径不变）。

## [0.7.10] - 2026-07-28

### Fixed

- **成稿页看不了也改不了**：带润色链生成时，成稿页顶上会常驻每轮润色的输出预览，而一轮输出就是整篇正文（五千多字），把下面的编辑器整个挤没了——用户看到的「成稿」其实是那块**只读**的预览，所以既改不了、也拉不到底、标题也不显示（预览里本来就没有标题）。现在成稿页和初稿页是**同一个编辑器**：白色编辑卡、可直接改、可滚动、首行显示文章标题。每轮润色的输出移到了字数那一行的「润色过程 N 轮」按钮里，点开是弹窗，「重跑此 pass」也在里面；重跑进行中时该按钮会显示「第 N 轮重跑中…」，关掉弹窗也看得见。
- ⚠️ **润色会把文章标题改掉**：标题本来是你定的（首页填的，或从候选里选的），但润色链只是把它当写作参考，没有任何东西拦着 AI 自己写一个标题、或者把原标题删掉——而正文首行的标题正是导出时认定的文章标题。现在润色**不允许改标题**：被改写会还原、被删掉会补回、被挪到正文中间会提回开头、AI 自己另写一个会换回你的。整篇润色、重跑单轮、横评三条路都生效。
- ⚠️ **AI 拟标题会把你输入的关键词改掉**：「换标题」候选一直是硬要求包含关键词的，但**没选标题**时（只填了关键词直接润色，这是最常见的用法）润色链完全不受约束，可以自己拟一个把关键词改掉甚至删掉的标题。现在这种情况下也有硬约束：AI 拟的标题必须**一字不差**包含你输入的关键词，做不到就不加标题、退回用关键词作标题。**口径变更**：过去 AI 自拟的标题即使不含关键词也会被当成文章标题，升级后这类标题会被去掉，成品文档的标题会变成你输入的关键词——想要更好的标题请用「换标题」挑一个。
- **首页填的标题被关键词顶掉**：在首页起飞条里填了标题时，编辑器首行和导出文档的标题显示的却是搜索关键词，你填的标题在稿子里根本看不见。
- **换标题后新标题跟不上**：换完标题再点「整篇润色」，AI 仍然围着旧标题写；点「重新随机」标题会被打回旧的；事实核对放行后导出的文件里也还是旧标题（界面显示新的、文件里是旧的）。现在这几条路都跟着新标题走。
- **纯文本导出的文件名和文中标题对不上**：在编辑器里直接改过标题那一行之后导出纯文本，文件名用的是改之前的标题、文件里第一行是改之后的标题。现在统一以正文里的标题为准（和 Markdown / Word 导出同口径）。
- **成稿改完再点「整篇润色」会被无声覆盖**：整篇润色是从毛坯文重新生成，成稿里的修改会被整段替换。以前编辑器根本改不了所以撞不到，现在能改了，会先问一句再继续。

## [0.7.9] - 2026-07-27

### Added

- **主推卡「从目录识别小节」**：竞品池早就有的那个按钮，主推卡现在也有了。主推品和竞品的素材形态**不是一回事**——竞品是一篇笔记装一整个竞品、小节写在正文里；主推品是**一个小节 = 一整篇笔记**，靠笔记属性（比如「模块」）区分。所以过去只能手敲小节名、再单独配一次筛选，两处对不上就出问题。现在点「从目录识别」会扫这个目录，自动认出该按哪个属性拆、拆出哪些小节、每节对应的笔记有没有正文，并按笔记里的序号排好顺序；勾选后「按目录替换」或「只补未配置的」。识别只读不落库，先看再导。**已经配好小节的块，默认只勾对得上现有小节的那些**——不然在本来正常的版本上点一下，会把目录里所有笔记（包括不该当正文的「标题行」）都加进去，等于把能用的版本改坏；没勾上的仍逐条列在面板上，要用自己勾。
- **结构检查新增三条提示**：①「小节没有配筛选」——这种小节会命中**整个目录随机抽一篇**，不报错但内容全凭运气（目录里只有一篇笔记时才是合法写法）；②「两节的目录和筛选完全相同」——会从同一个池各抽一次，成文里出现两段同类内容；③「主推块没有配小节、后面也没有可用段落」——每篇只会输出一行块名加一行「推荐理由」、没有正文，多半是改成卡片模式后忘了删的旧块。生成时如果真抽到了没配筛选的小节，也会在结果里说明。

### Fixed

- ⚠️ **导出的文档没有标题**：成稿正文里本来就不含标题（标题是单独一个字段，只在编辑器里临时显示成首行），而导出发的是不带标题的正文，于是导出的 Markdown / Word 通篇没有标题，首页「最近文档」的标题列显示的也是第一个章节名（「一、品牌分析」这种）。现在导出会自动把标题补成正文首行，生成结束的自动导出、事实核对放行后的导出、历史索引镜像三条路径一并生效。**口径变更**：升级后导出的文档会多出标题这一行，因此字数统计比旧文档略多；首页「最近文档」的标题列也会从章节名变成真实标题，新旧文档在这两处不可直接对比。
- **Word 导出里标题变成字面的 `##`**：过去只有标题独占一段（前后都有空行）才会被识别成 Word 标题，而 AI 润色返回的正文经常是「## 一、品牌分析」紧接着正文第一句、中间没有空行，整块就被当成普通段落，打开 Word 看到的是字面的 `##`。现在逐行识别，标题该是标题、正文该是正文。
- **润色过程的预览显示成 md 源码**：成稿页上方每轮润色的输出预览此前是纯文本，同一段正文在下面的编辑器里排得好好的、在预览里却是满屏 `##` 和 `**`，看起来就像「润色把排版弄没了」。现在预览也按排版渲染。
- **导出纯文本会出现两个标题**：纯文本导出无条件在正文前面又拼一次标题，而正文里往往已经有标题了（在成稿编辑器里编辑过一次就会）；改过标题文字的话，出来的还是两个**不一样**的标题。
- **换标题后选了不生效**：正文里一旦已经带上标题，再从候选里换一个新标题时，编辑器和导出都还显示旧的。
- **命令行 `csm` 直接崩溃**：`csm` / `python -m csm_core` 这条命令行生成会以 `KeyError` 中断，一篇也产不出来（图形界面不受影响，走的是另一条路径）。

### Notes

- 这次升级**不会自动修好已经配坏的模板**。如果你的某个版本此前生成报「没有符合条件的素材」，需要在模板编辑器里对那张主推卡点一次「从目录识别 → 按目录替换」；面板上没有自动勾选的取值，确认要用的自己勾上。

## [0.7.8] - 2026-07-26

### Fixed

- **百度抓取导入 Chrome profile 报「未找到 …\\User Data\\Default」**：导入流程此前写死复制名为 `Default` 的 profile，而 Chrome 里删过默认账号、只剩 `Profile 1`/`Profile 2` 的机器上根本没有这个目录，于是导入必然失败，报错还是一句没有下文的英文路径，界面上也没有任何地方能改选。现在：① 复制哪个 profile 由实际存在的目录决定（上次选过的 → `Default` → Chrome 上次用的 → 唯一的那个 → 最近活跃的）；② 设置页新增「要复制的 Chrome profile」下拉，列出机器上真实的 profile（带 Chrome 里的备注名和登录邮箱），多账号用户可自己选，导入结果也会写明实际复制的是哪个；③ Chrome 数据目录不再只认默认位置，会一并查组策略指定的目录和 Beta / Dev / Canary / Chromium，且优先选**里面真的有 profile** 的那个（Chrome 装了从没启动过时默认目录是空的）；④ 设置页新增「Chrome 数据目录」输入框，公司统一部署、换过盘等探测不到的情况可以手填（带引号或写 `%LOCALAPPDATA%` 的路径都能识别；填的目录里没有 profile 时会自动退回探测结果并说明换用了哪个目录，不会一填错就卡死）；⑤ 失败提示改成中文并说清下一步——是目录不存在、还是没有读取权限（公司统管的电脑常见）、还是目录里没有任何 profile（提示先启动一次 Chrome）、还是多填了一层（填到了某个 profile 目录里）、还是选的 profile 不存在（直接列出可选的有哪些）。另外：自动填入的 Chrome 程序路径现在会跟数据目录的版本对应（用 Beta 的 profile 就配 Beta 的 Chrome，避免副本因「配置文件来自更新版本」打不开）；把 CSM 自己的副本目录误填进「Chrome 数据目录」会被拦下，不会把已导入的副本连同副本里的百度登录态一起清空。**已经导入成功的用户不受影响**——你的 `Default` 还在，升级后复制的仍然是同一个 profile，不需要重新导入。
