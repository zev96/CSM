...
- ⚠️ **口径变更 · 筛选条件的匹配放宽**：模板筛选此前是严格相等，于是两类资料库里很常见的写法**永远筛不出东西**——标签列表（`核心关键词: [模板二, 主推位, 品牌实力]`，选「品牌实力」匹配不上整个列表）和数字（`模板序号: 2` 是数字，填的「2」是文本）。现在列表按「含这个标签」判定、标量按文本比对。原本就能用的筛选不受影响（上面两类以前恒为空池、直接报错，没有模板能依赖它），但**如果你的资料库里有列表或数字字段，用到它们的筛选会从「一篇都筛不出」变成「筛得出」**，相关块的出稿内容会变。
