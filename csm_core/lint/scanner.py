...
_SENT_BOUND = set("。！？!?\n")
# 去重排序的最后一道 tiebreak —— 仅当两命中同起点同长度时才决胜。默认六类词表彼此
# 不在同一起点开头，唯有用户 extra_* 词与默认词在同一位置重叠时才会触发。
...
def _word_hits(text: str, words: tuple[str, ...], category: str) -> list[tuple]:
    out = []
    for w in words:
        i = text.find(w)
        while i != -1:
...
def autofix(text: str, rules: Rules) -> str:
    """只清机械三类：emoji 删 / 双引号删（保留内文）/ 破折号→逗号。判断类不动。幂等。

    顺序铁律：引号删除必须先于破折号→逗号的合并清理。否则「——“——」里引号还在时
    两段破折号各自塌成一个逗号，引号删完才让两逗号相邻，已跑完的「，{2,}」合并看不到
    它们 → 非幂等（spec §4.3）。先删引号 → 两段破折号并成一段 → 一次塌成单逗号。
    """
    t = text
    if rules.check_emoji:
    ...
    if rules.check_dash:
        ...
        t = re.sub(r"，([。！？!?])", r"\1", t)   # 句末标点前多余逗号去掉（对称）
    return t
