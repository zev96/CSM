...
> **勘误（Task 3 质量审查收口，commit 84bd82c，以最终代码为准）**：①下方 3c 片段的 `activeJob.value?.id === d.job_id` 过不了 vue-tsc strict（TS18047，d.job_id 为 undefined 时无法收窄 null）——最终代码用 `const aj = activeJob.value; aj && aj.id === d.job_id`；②mining `st === "cancelled"`、batch `by_status.cancelled > 0` 时**静默不推**（用户主动取消，与 monitor/article 取消静默一致）；③monitor_done 的 body total：后端 finished 事件不带 progress_total，须在 clearRunning 前从 `progressOf(id)?.total` 抢救，没有则省略「共 N 项」段；④monitor_done hint 标注「含定时」。
...
> **勘误（Task 4 质量审查收口，以最终代码为准）**：①batch/article 卡必须 gate 在 `jobId` 非空上（submit 先同步置 running、POST 返回才有 jobId——`*:pending` 中间 key 会被 watcher 误登记为幽灵「已完成」条目）；②`_outcomeFor` 的 mining 分支加「activeJob 还是不是这张卡」守卫（batch/article 不加——terminal status 与卡片消失同 flush，且 teardown 会清 jobId，加了反而误判）；③`PLATFORM_LABEL[p] ?? p` 兜底；④monitorStatus `started` 清同 id 旧 `lastOutcomes`（重跑不背旧账）。
...
describe("taskTray — 监测聚合", () => {
  ...
  it("waiting_chrome phase → state=waiting + 排队文案；captcha 优先级更高", async () => {
    ...
    monitor._dispatchSse("needs_captcha", { task_id: 31, keyword: "kw" }); // 实发事件（Task 2 勘误）
    await nextTick();
...
- [x] **Step 3: 实现 taskTray.ts（本任务先落监测部分，mining/batch/article 的 computed 返回 null 占位由 Task 5 填充——文件一次写全也可以，则 Task 5 Step 2 直接绿）**
...
- [x] **Step 3: 实现 generate_service.py**
...
- [x] **Step 4: 实现 routes/generate.py —— 文件末尾追加**
