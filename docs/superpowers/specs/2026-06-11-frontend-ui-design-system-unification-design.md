...
### 4.2 左栏：瘦成 GEO 式导航
- **L1 任务列表**：知乎/评论/百度 L1 本就约 3 列（`名称 | 计数/状态 | 操作`），与 GEO 一致，仅随 340px 微调。
- **L2 列表**（问题 / 视频 / 关键词结果，现 4–5 列）：瘦成 `名称 + 一个关键状态(如卡位 Pill) + 操作` 的导航行；次要指标（浏览量等）降为名称下副标，其余（变化 / 趋势 / 平台）移到右栏。
- 列宽统一向 GEO 的 `1.5fr .9fr 1.1fr` 口径靠拢。

### 4.3 右栏：承接指标 + 放大详情
变宽到 ~70% 后，右详情参考 GEO 的 `GeoKeywordDetail` 结构、**逐页定制**（非通用模板）：
- 顶部 KPI 卡（卡位 / 变化 / 浏览量等从左栏移来的指标）
- 更大的趋势图（原来挤在 42% 里的折线放大）
- 视情况补平台分布条 / 信源列表等

逐页右栏内容：知乎=问题卡位详情、评论=保级/盖楼详情、百度=排名详情、知乎搜索=关键词结果详情。
