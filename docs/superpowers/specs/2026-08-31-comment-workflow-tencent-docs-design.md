...
- **按平台路由子表**（2026-09-01 补充确认：用户表格按平台分 tab —— 抖音/B站/快手/小红书）：`sheet_map` 配置平台 → 子表名（默认 抖音/B站/快手），同步时按子表名去空白匹配路由；找不到同名子表回落 URL tab / 第一张子表。每平台一个写入块，序号各自从 1 重排。
- **一批 = 一次同步**：追加 N 行，`序号` 从 1 编号，`日期` 写当天（`2026年9月1日` 格式，对齐表内最新批次）；**批间分隔行**：子表已有数据行时先留一行空行并涂橙色底（`sheet.set_cell_style` bg=FFFFC000，失败不阻塞）；上一批留下的空分隔行会被复用，不会双空行。
