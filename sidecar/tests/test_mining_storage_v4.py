...
def test_schema_version_is_4(fresh_db: Path):
    # The check is really "schema_meta tracks the current stamp" — compare
    # against monitor_storage._SCHEMA_VERSION so schema bumps don't break it.
...
def test_v3_to_v4_upgrade_preserves_video_rows(tmp_path: Path, monkeypatch):
    ...
    # schema_meta tracks the CURRENT global version stamp — compare against
    # monitor_storage._SCHEMA_VERSION so schema bumps don't break this test.
    ...
    assert ver == str(monitor_storage._SCHEMA_VERSION)
