...
# ── Historical reports (monitor screen 历史报告 tab) ───────────────────────
def get_reports(period: str = "daily", limit: int = 30) -> dict[str, Any]:
    """Aggregate alert-triggered results by day or week.

    Because csm_core.monitor.storage exposes no global query, we reach
    into the connection here directly — simpler than adding a new public
    helper to csm_core for one UI screen.
    """
    if period not in ("daily", "weekly"):
        raise ValueError(f"period must be 'daily' or 'weekly', got {period!r}")
    conn = storage.get_conn()
    # Pull raw rows then bucket in Python — sqlite's date arithmetic is
    # inconsistent across drivers and we already serialize datetimes
    # ourselves.
    cutoff_days = 30 if period == "daily" else 90
    ...
    rows = conn.execute(
        ...
        """,
        (storage._format_iso(cutoff),),  # noqa: SLF001 — internal helper, but stable
    ).fetchall()

    buckets: dict[str, dict[str, Any]] = {}
    for row in rows:
        checked_at = storage._parse_iso(row["checked_at"]) or datetime.now()  # noqa: SLF001
        bucket_key = _bucket_key(checked_at, period)
